...
def inferFirst(rel_graph,all_rel,inds, C):
    ...
    for node in rel_graph.nodes():
        if not node in checked:
            ...
            for sib in siblings:
                checked.append(sib)
            [add_sibs,remove] = checkSiblingSubgraph(rel_graph,siblings[:]) #edit siblings list, return removed sibs
            ...
            neighbors = rel_graph.neighbors(siblings[0])
            inds_to_check = []
            for n in neighbors:
                if rel_graph.get_edge_data(siblings[0],n)['type'] == '1U':
                    inds_to_check.append(n)
...
def inferFirstFaminfo(rel_graph, all_rel, inds, C):
    ...
    for node in rel_graph_tmp.nodes():
        if not node in checked:
            ...
            for sib in siblings:
                checked.append(sib)
            [add_sibs,remove] = checkSiblingSubgraph(rel_graph_tmp,siblings[:]) #edit siblings list, return removed sibs
        ...
        if not rel_graph.has_edge(ind1, ind2) or (force == 1 and not not rel_graph.get_edge_data(ind1,ind2)['type'] in ['NN','AU']):
            ...
            if i1 in all_rel.keys() and i2 in all_rel[i1].keys():
                ...
                if float(all_rel[i1][i2][3]) == 2:
                    if float(all_rel[i1][i2][1]) < 0.01: #IBD1, IBD2, K, D
                        tmp = i1+'_'+i2
                        rel_graph.add_edge(i1,i2)
                        rel_graph.add_edge(i2,i1)
    ...
    for node in rel_graph.nodes():
        if not node in checked:
            ...
            if len(sibs) > 1:
                ...
                [avunc, avunc_hs1, avunc_hs2] = getAuntsUncles_IBD011_nonoverlapping_pairs(all_rel, sibs, halfsibs, second, file_for_segments, rel_graph, outfile)
                print("Done")
...
def checkForMoveUp(ind, sibset, older_gen, third_party):
                                    ...
                                    if checkOverlap(range_add, ranges[chr][krange]):
                                        ...
                                        if range_add[1] > ranges[chr][krange][1]:  # new range ends after krange
                                            if krange < len(ranges[chr]) - 1:
                                                ...
                                                if new_range[0] > new_range[1]:
                                                    print(sib1+'\t'+sib2+'\t'+av+'\t'+str(chr) + '\t' + str(ranges[chr][krange][1]) + '\t' + str(
...
def getSiblingRelativeFamIBDLengthIBD2(sib1, sib2, avunc1, avunc2, file_for_segments):
    ...
    sibandav = sib1[:]
    for avunc in avunc1:
        sibandav.append(avunc)

    sibandav_rel = sib2[:]
    for avunc in avunc2:
        sibandav_rel.append(avunc)
    ...
    for ind in inds:
        if not ind in sibset:
            ...
            while k < len(sibset) and deg == 2:
                if ind < sibset[k]:
                    deg = all_rel[ind][sibset[k]][3]
                else:
                    deg = all_rel[sibset[k]][ind][3]
                k = k + 1
...
def getSecondDegreeRelativesFromAllRel_lessRestrictive(all_rel,inds,sibset):
    # collect all second and third degree relatives (to check for aunts/uncles later)
    second = []
    for ind in inds:
        if not ind in sibset:
            degs = []
            for k in range(0,len(sibset)):
                if ind < sibset[k]:
                    degs.append(all_rel[ind][sibset[k]][3])
                else:
                    degs.append(all_rel[sibset[k]][ind][3])
...
def getAuntsUncles_IBD011_nonoverlapping_pairs(all_rel, sibset, halfsibs, second, file_for_segments, rel_graph, output):
    # check whether individuals in list 'second' are likely aunts/uncles of 'sibset' and possibly also 'halfsibs'
    avunc = []
    remove_second = []
    #allsibs = sibset + halfsibs
    avunc_hs1 = []
    avunc_hs2 = []
    if len(second):
        for ind in second:
        ...
        if len(second):
            ...
            for i in range(0, numpairs):
                ...
                is_av=0
                for av in second:
                    #todo: remove the following comment, indent
                    #if not av in avunc:
                    ...
                    IBD2 = []
                    if av < sibset[(i * 2)]:
                        IBD2.append(str(all_rel[av][sibset[(i * 2)]][1]))
                    else:
                        IBD2.append(str(all_rel[sibset[(i * 2)]][av][1]))
                    if av < sibset[(i*2+1)]:
                        IBD2.append(str(all_rel[av][sibset[(i * 2+1)]][1]))
                    else:
                        IBD2.append(str(all_rel[sibset[(i * 2+1)]][av][1]))
                    if IBD011 > 50:
                        avunc.append(av)
            ...
            if len(halfsibs):
                ...
                for i in range(0, numpairs):
                    ...
                    for av in second:
                        ...
                        IBD2 = []
                        if av < allsibs[(i * 2)]:
                            IBD2.append(str(all_rel[av][allsibs[(i * 2)]][1]))
                        else:
                            IBD2.append(str(all_rel[allsibs[(i * 2)]][av][1]))
                        if av < allsibs[(i * 2 + 1)]:
                            IBD2.append(str(all_rel[av][allsibs[(i * 2 + 1)]][1]))
                        else:
                            IBD2.append(str(all_rel[allsibs[(i * 2 + 1)]][av][1]))
                        if IBD011 > 50:
                            avunc_hs1.append(av)
                            avsibs = getSibsFromGraph(rel_graph, av)
                            for avsib in avsibs:
                ...
                if len(halfsibs) > 1 and len(halfsibs[1]):
                    file = open(output + '_2nd_IBD011.txt', 'a')
    ...
    for [ind1,ind2] in itertools.combinations(inds,2): #test each pair of individuals
        if not [ind1,ind2] in checked and not [ind2,ind1] in checked: #if pair not yet tested
            ...
            if rel_graph.has_edge(ind1, ind2):
                ...
                if type == '1U':
                    type = '1'
                results = results + [[ind1, ind2, type, refined]]
            else:
                ...
                if reltype != -1:
                    ...
                    ind2_original = ind2
                    ind2_new = checkForMoveUp(ind2,sib2,par2+gp2,sib1)
                    moves2 = []
                    moves_inds2 = []
                    while ind2 != ind2_new:
                    ...
                    if not len(avunc1_bothsides) and len(nn1):
                        tmp = sib1[:]
                        sib1 = nn1[:]
                        avunc1_bothsides = [tmp[:]]
                    if not len(avunc2_bothsides) and len(nn2):
                        tmp = sib2[:]
                        sib2 = nn2[:]
                    ...
                    if len(avunc1_bothsides) or len(avunc2_bothsides):
                        ...
                        all_avunc2 = []
                        for i in range(0, len(avunc1_bothsides)):
                            all_avunc1 = all_avunc1 + avunc1_bothsides[i]
                        for i in range(0, len(avunc2_bothsides)):
                            all_avunc2 = all_avunc2 + avunc2_bothsides[i]
                    ...
                    results = combineBothGPsKeepProportionOnlyExpectation_final(sib1, relavunc1, sib2, relavunc2, -1, -1, args.s[0], args.i[0], rel_graph)
                ...
                if len(twins1):
                    for res in results:
                        if sibs1[0] == res[0]:
                            for t1 in twins1:
                                results.append([t1,res[1],res[2],res[3]])
                        elif sibs1[0] == res[1]:
                            for t1 in twins1:
                                results.append(res[0],t1,res[2],res[3])

                if len(twins2):
                    for res in results:
                        if sibs2[0] == res[0]:
                            for t2 in twins2:
                                results.append([t2, res[1], res[2], res[3]])
                        elif sibs2[0] == res[1]:
                            for t2 in twins2:
                                results.append(res[0], t2, res[2], res[3])
